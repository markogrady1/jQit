 ...
 * @param callback
 */
resolve.getTeamData = function(db, target,  teams, callback) {
	var arr;
	var index = -1;
	setTimeout(() => {
		callback(arr)
	...
	while(++index < teams.length) {
		console.log("in function",teams[index])
		this.getHistoryQuery("repoPullsHistory", "pulls", teams[index], (obj) => {
			arr = (obj)
		});
	}
 ...
 * @param {Function} callback
 */
resolve.getHistoryQuery = function(database, target, param, callback) {
