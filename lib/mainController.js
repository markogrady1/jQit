 ...
 * @param {Object} res
 */
resolve.resolveTeamData = function(team, req, res, callback) {
	var teamRepos = [], repoData = [];
	...
	for (var i in teams) {
		if (team === teams[i].team) {
			for (var j in teams[i].responsibility) {
				...
				getHistoryQuery("repoPullsHistory", "pulls", teams[i].responsibility[j], (obj) => {
console.log(obj)
					console.log(obj)
