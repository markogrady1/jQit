...
resolve.resolveIssueData = function(param, req, res) {
	...
	function getFlagData(callback){
		if (userData !== "undefined") {
			var name = helper.getSplitValue(userData, "=>", 0);
			var email = helper.getSplitValue(userData, "=>", 2);
	...
	var clIssues = "";
 	schema.getRecord(nameParam, (doc) => {
		...
		//getClosedPullsNo(nameParam, (pullClosedNum, pullDat) => {
		//	pullClosedNum = pullClosedNum;
		//});
...
//function getClosedPullsNo(nameParam, callback) {
//	schema.executeClosedNoQuery("repoClosedPullsHistory", nameParam, (closedPullDoc) => {
//
//		callback(closedPullDoc[closedPullDoc.length-1].pulls, closedPullDoc);
//	});
//	}
...
resolve.getFlagData = function(userObj, callback) {
    var name = helper.getSplitValue(userObj, "=>", 0);
    var email = helper.getSplitValue(userObj, "=>", 2);
    var avatar = helper.getSplitValue(userObj, "=>", 1);
...
resolve.getStorageItems = function(item, index) {
