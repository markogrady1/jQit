 	...
 	schema.getRecord(nameParam, function(doc){
    	...
    	getClosedNo(nameParam, function(pullClosedNumber, pullDat, issueClosedNumber, issueDoc){
			...
			for (var i in pullDoc) {
				df.dateTimes(pullDat[i].created_at, pullDat[i].closed_at, false, function(val){
					timeScale += val;
					
				});
				// val = df.DateDifference(pullDat[i].created_at, pullDat[i].closed_at, true);
				
			};
			
			diff = timeScale / pullDoc.length;
			...
			pullTimeString = dd + " days " + hh + " hours " + mm + " minutes " + ss + " seconds";
			if(pullDoc.length < 3){
				pullTimeString = "0 Days";
			}
