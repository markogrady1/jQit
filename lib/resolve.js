...
exports.validateRegistration = function(req, res) {
	...
	if (username == '' || userEmail == '' || regPass  == '' || regConfirmPass == '' ) {
		schema.setStatus('Empty fields');
		return false;
	} else {
		if (regPass !== regConfirmPass) {
			schema.setStatus('Passwords did not match');
			return false;
		} else {
			schema.regUser(username, userEmail, regPass, res);
			schema.setStatus('Register');
...
exports.validateLogin = function(req, res, fn) {
	...
	if(uemail === '' || password === '') {
		schema.setLoginStatus('Empty fields');
		fn(false);
	} else {
		schema.loginUser(uemail, password, function(reslt, data) {
			if(!reslt) 
				schema.setLoginStatus('Incorrect email or password');
			fn(reslt, data);
		});
	}
}
