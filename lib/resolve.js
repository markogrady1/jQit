 	...
 	schema.getRecord(nameParam, function(doc){
    	...
    	getClosedNo(nameParam, function(pullClosedNumber, pullDat, issueClosedNumber, issueDat){
			...
			for (var i in pullDoc) {
				var notDiff = df.timeFormat(true);
				notDiff(pullDat[i].created_at, pullDat[i].closed_at, function(pVal){
					pullTimeScale += pVal;
				});
			}
			...
			for(var j in issueDoc){
				var notDiff = df.timeFormat(true);
				notDiff(issueDat[j].created_at, issueDat[j].closed_at, function(iVal){
					issueTimeScale += iVal;
				});
			}
			...
			var isDiff = df.timeFormat(false);
			isDiff(pDiff, null, function(pval){
					pullTimeString = pval;
			});
			...
			var isDiff = df.timeFormat(false);
			isDiff(iDiff, null, function(ival){
...
function eventsData(param, cb){
	schema.getEvents(param, function(d){
    	ev = d;
    	cb(ev);
    });
}
