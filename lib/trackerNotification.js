...
trackerNotif.getUserDetails = function() {
    ...
    var email;
    var flagObject = {};
    migrate = require("../models/history");
    var userFlagData = [];
    notificationModel.getAllUsers((userData) => {
        async.each(userData, function (item, callback) {
            ...
            email = item.email;
            notificationModel.getEmailAlerts(username, email, (userEmailNotifs) => {
                notificationModel.getTeamEmailAlerts(username, email, (teamEmailNotifs) => {
                    notificationModel.getContentEmailAlerts(username, email, (contentEmailNotifs) => {
                        if (teamEmailNotifs.length !== 0) {

                            userEmailNotifs.team_target = teamEmailNotifs[0].team_target;
                            userEmailNotifs.issues_team_boundary = teamEmailNotifs[0].issues_team_boundary;
                            userEmailNotifs.pulls_team_boundary = teamEmailNotifs[0].pulls_team_boundary;
                        }
                        if (contentEmailNotifs.length !== 0) {
                            userEmailNotifs.content_target = contentEmailNotifs[0].content_team_target;
                            userEmailNotifs.issues_content_team_boundary = contentEmailNotifs[0].issues_content_team_boundary;
                            userEmailNotifs.pulls_content_team_boundary = contentEmailNotifs[0].pulls_content_team_boundary;
                        }
            ...
            if (userFlagData.length > 0) {
                var rStr, tStr,cStr;
                trackerNotif.getRepoHistory(userFlagData[0], (repoStr) => {
                    trackerNotif.getTeamRepoHistory(userFlagData[0], (teamStr) => {
                        trackerNotif.getContentTeamRepoHistory(userFlagData[0], (contentStr) => {

                            if(repoStr !== "") {
                                rStr = "<li class='head-list'><h3>Repository Watch</h3></li>" + repoStr;
                            } else {
                                rStr = "";
                            }
                            if(teamStr !== "") {
                                tStr = "<li class='head-list'><h3>Team Watch</h3></li>" + teamStr;
                            } else {
                                tStr = "";
                            }
                            if(contentStr !== "") {
                                cStr = "<li class='head-list'><h3>Content Team Watch</h3></li>" + contentStr;
                            } else {
                                cStr = "";
                            }
                            var mailString = rStr + tStr + cStr;
                            if(mailString !== "") {
                                trackerNotif.sendNotification(email, mailString, rStr, tStr, cStr, username)
                            }
...
trackerNotif.getRepoHistory = function(flagData, cb) {
    ...
    async.each(flagData, function(item, callback){
        ...
        if(item.username !== "none") {
            ...
            data.issues_boundary = item.issues_boundary;
            data.pulls_boundary = item.pulls_boundary;
            notificationModel.getRepoIssueHistory(data.target, (issueHistory) => {
                var dateStr = issueHistory[issueHistory.length-1].secondsDate;
                dateStr = new Date(dateStr*1000).toDateString();
                var dateArr = dateStr.split(" ");
                dateStr = dateArr[0] + " " + dateArr[1] + " " + dateArr[2] + " " + dateArr[3];
                data.dateOfCheck = dateStr;
            ...
            data.avatar =  "none";
            data.target =  "none";
            data.issues_boundary = 1;
            data.pulls_boundary = 1;
            data.dateOfCheck = "none";
            data.lastIssue = 0;
            data.secondlastIssue = 0
            data.lastPR = 0;
            data.secondlastPR = 0;
            targetData.push(data);
            callback();
        ...
        trackerNotif.checkForDataIncrease(targetData, (singleStrings) => {
            cb(singleStrings)
        });
    });
};

trackerNotif.getTeamRepoHistory = function(flagData, cb) {
    var teamRepos = [];
    if(flagData.username !== "none") {
        var teams = require("../repoData/teams.json");
        for (var i in teams) {
            if (flagData.team_target  === teams[i].team) {
        ...
        async.each(teamRepos, function(item, callback){
            var data = {};
            data.username = flagData[0].username;
            data.email = flagData[0].email;
            data.avatar = flagData[0].avatar;
            data.target = item;
            data.issues_boundary = flagData.issues_team_boundary;
            data.pulls_boundary = flagData.pulls_team_boundary;
            notificationModel.getRepoIssueHistory(item, (issueHistory) => {
                var dateStr = issueHistory[issueHistory.length-1].secondsDate;
                dateStr = new Date(dateStr*1000).toDateString();
                var dateArr = dateStr.split(" ");
                dateStr = dateArr[0] + " " + dateArr[1] + " " + dateArr[2] + " " + dateArr[3];
            ...
            trackerNotif.checkForTeamDataIncrease(targetData, (emailStrings) => {
                cb(emailStrings)
            });
        });
    } else {
...
trackerNotif.getContentTeamRepoHistory = function(flagData, cb) {
    if(flagData.username !== "none" ) {
        var teamRepos = [];
        var teams = require("../repoData/content_team.json");
        for (var i in teams) {
            if (flagData.content_target === teams[i].team) {
        ...
        async.each(teamRepos, function (item, callback) {
            ...
            data.avatar = flagData[0].avatar;
            data.target = item;
            data.issues_boundary = flagData.issues_content_team_boundary;
            data.pulls_boundary = flagData.pulls_content_team_boundary;
            notificationModel.getRepoIssueHistory(item, (issueHistory) => {
                var dateStr = issueHistory[issueHistory.length - 1].secondsDate;
                dateStr = new Date(dateStr * 1000).toDateString();
                var dateArr = dateStr.split(" ");
            ...
            trackerNotif.checkForContentTeamDataIncrease(targetData, (emailStrings) => {
                cb(emailStrings)
            });
        });
    }else {
...
trackerNotif.checkForTeamDataIncrease = function(targetData, cb) {
    var notificationStr = '';
    async.each(targetData, function(item, callback){
        var issueValue = parseInt(item.lastIssue) -  parseInt(item.secondlastIssue)
        var pullsValue = item.lastPR - item.secondlastPR
        if(issueValue !== 0){

            if(issueValue >= item.issues_boundary) {
                notificationStr += "<li>" + item.target + "  saw an increase of " + issueValue + " issues on " + item.dateOfCheck + "</li>"
            } else {
                notificationStr += "";
            }
        } else {
        ...
        if( pullsValue !== 0){
            if(pullsValue >= item.pulls_boundary) {
                notificationStr +="<li>" + item.target + "  saw an increase of " + pullsValue + " pull requests on " + item.dateOfCheck + "</li>"
            } else {
                notificationStr += "";
            }
        } else {
...
trackerNotif.checkForContentTeamDataIncrease = function(targetData, cb) {
    var notificationStr = '';
    async.each(targetData, function(item, callback){
        var issueValue = parseInt(item.lastIssue) -  parseInt(item.secondlastIssue)
        var pullsValue = item.lastPR - item.secondlastPR
...
trackerNotif.checkForDataIncrease = function(targetData, cb) {
    var notificationStr = '';
    var prNotificationStr = '';
    if(targetData[0].username !== "none") {
        var issueValue = parseInt(targetData[0].lastIssue) -  parseInt(targetData[0].secondlastIssue)
        var pullsValue = targetData[0].lastPR - targetData[0].secondlastPR
        if( issueValue !== 0) {
            if(issueValue >= targetData[0].issues_boundary) {
                notificationStr += "<li>" + targetData[0].target + "  saw an increase of " + issueValue + " issues on " + targetData[0].dateOfCheck + "</li>"
            } else {
                notificationStr += "";
            }
        } else {
        ...
        if(pullsValue !== 0) {
            if(pullsValue >= targetData[0].pulls_boundary) {
                notificationStr +="<li>" + targetData[0].target + "  saw an increase of " + pullsValue + " pull requests on " + targetData[0].dateOfCheck + "</li>"
            } else {
                notificationStr += "";
            }
        } else {
...
trackerNotif.sendNotification = function(email, mailString, repoStr, teamStr, contStr, username) {
    ...
    var mailOptions = {
        ...
        "<tr>"+
        "<th>"+
        "<h1 class='text-center'>jQit Issue tracker</h1>"+
        ...
        "</tr>"+
        "</table>"+
        "<h4 class='title'>Tracking Data</h4>"+
        "<h4></h4>"+
        "<br><br>" +"" +
        "<p>Dear " + user + "<br><br><ul>" + repoStr + "   <br>" + teamStr +  "   <br>" + contStr + "</ul></p>"+
        "<table class='menu'>"+
        "<tr>"+
        "<td>"+
        "<table>"+
        "<tr>"+
        "<th class='menu-item float-center'>"+"<a href='#'>jqit.com</a></th>"+
