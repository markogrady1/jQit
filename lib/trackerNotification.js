    ...
    notificationModel.getAllUsers((userData) => {
        async.each(userData, function (item, callback) {
            ...
            if (userFlagData.length > 0) {
                trackerNotif.getRepoHistory(userFlagData[0]);
            }
        });
...
trackerNotif.getRepoHistory = function(flagData) {
    ...
    async.each(flagData, function(item, callback){
        ...
        trackerNotif.checkForDataIncrease(targetData);
...
trackerNotif.checkForDataIncrease = function(targetData) {
    var issueNotificationStr = '';
    var prNotificationStr = '';
    console.log(targetData)
console.log(targetData[0].lastIssue, targetData[0].secondlastIssue)
    ...
    if(issueValue >= targetData[0].issues_boundary) {
        issueNotificationStr = "<li>" + targetData[0].target + "  saw an increase of " + issueValue + " issues on " + targetData[0].dateOfCheck + "</li>"
    } else {
        issueNotificationStr = "";
    }

    if(pullsValue >= targetData[0].pulls_boundary) {
        prNotificationStr ="<li>" + targetData[0].target + "  saw an increase of " + pullsValue + " pull requests on " + targetData[0].dateOfCheck + "</li>"
    } else {
        prNotificationStr = "";
    }
    trackerNotif.sendNotification(issueNotificationStr, prNotificationStr, targetData[0]);
    //console.log(issueNotificationStr, prNotificationStr)
}



trackerNotif.sendNotification = function(issueStr, prStr, targetData) {
    // create reusable transporter object using the default SMTP transport

var user = targetData.username;
...
var mailOptions = {
        ...
        "" +
        ".info-txt{color:#2e4677} a { color: #336699; } " +
    ...
    "<th class='menu-item float-center'><a href='#'></a></th>"+
        ...
        "<h4></h4>"+
        "<br><br>" +"" +
    "<p>Dear " + user + "<br><br><ul>" + issueStr + "   <br>" + prStr + "</ul></p>"+
    "<table class='menu'>"+
        "<tr>"+
        "<td>"+
        "<table>"+
