...
var schema = require("../schema/repository")
 ...
 */
monthly.getPRs = function(target, callback) {
    var self = this;
 ...
 */
monthly.formatPRs = function(chunk) {
    var repoDt;
    var pullsData = [];
...
/**
 * Function responsible for resolving the team data and acquiring the view
 *
 * @param {String} team
 * @param {Object} req
 * @param {Object} res
...
monthly.getTeamData = function(db, target,  teams, callback) {
    var d ;
    console.log(teams)
    schema.executeQuery(db, teams, (data) => {
        d = data
    });
    setTimeout(() => {
        callback(d)
 ...
 * @param callback
 */
monthly.getNextIssueMonth = function(repo, range, callback) {
 ...
 * @param callback
 */
monthly.getNextPullsMonth = function(repo, range, callback) {
 ...
 * @param {Object} res
 */
monthly.getNewMonth = function(param, range,  req, res, io, callback) {
    helper.noCache(res);
    io.emit("start")
    ...
    this.getNextMonthHistoryQuery("repoPullsHistory", "pulls", param, (obj) => {
        pullRaw = obj.raw;
        pullChartData = obj.chart;
        pullNumbers = obj.numbers;
        repoPullHistory = obj.historyData;
    ...
    this.getNextMonthHistoryQuery("repoHistory", "issues", param, (obj) => {
        fullDateRaw = obj.raw;
        chartData = obj.chart;
        issueNumbers = obj.numbers;
        repoHistory = obj.historyData;
 ...
 * @param {Number} range
 */
monthly.getHistoryQuery = function(database, target, param, callback, range) {
...
monthly.getNextMonthHistoryQuery = function(database, target, param, callback, range) {
    var numbers = [], historyData = [], jsonData = [], raw = [];
 ...
 * @param {Object} res
 */
monthly.resolveIssueDates = function(param, req, res) {
    nameParam = param;
 ...
 * @param {String} uname
 */
monthly.initiateLogin = function(req, res, uname) {
    username = uname
    res.render("login", {
 ...
 * @param {Object} obj
 */
monthly.initiateRegistration = function(req, res, state, obj, io) {
 ...
 * @param {Object} res
 */
monthly.validateRegistration = function(req, res) {
 ...
 * @param {Function} callback
 */
monthly.validateLogin = function(req, res, callback) {
 ...
 *
 */
monthly.checkForAssigneeAddition = function() {
    var details;
 ...
 * @param {Function} callback
 */
monthly.getFlagData = function(userObj, callback) {
 ...
 * @param {Object} data
 */
monthly.cacheRepoData = function(data) {
    dataCache = data;
};
 ...
 * @param {Object} obj
 */
monthly.assignWatcher = function(obj) {
 ...
 * @returns {String}
 */
monthly.getAvatarImage = function() {
    var data = localStorage.getItem("data");
    if(data === " ") {
        return "undefined";
    } else {
...
monthly.getStorageItem = function(index) {
    var data = localStorage.getItem("data");
    if(data === " ") {
        return "undefined";
    } else {
 ...
 */
monthly.removeLoggedInStatusofUser = function() {
